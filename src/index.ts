  ...
  const register = () => {
    ctx.helper.afterUploadPlugins.register('cloudflare-deploy-hook', {
      handle: async function (ctx) {
        ...
        // 设置类型
        const requestHeaders: string = ctx.getConfig('picgo-plugin-cloudflare-deploy-hook.requestHeaders') || '{}';
        const requestBody: string = ctx.getConfig('picgo-plugin-cloudflare-deploy-hook.requestBody') || '';
            ...
            const sendRequest = async () => {
              try {
                ...
                const response = await axios({
                  method: requestMethod as string,  // 强制断言为 string 类型
                  url: requestUrl as string,  // 强制断言为 string 类型
                  headers: headers,
                  data: body
                });
